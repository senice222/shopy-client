...
const Pagination: React.FC<PaginationProps> = ({ totalPages, currentPage, onPageChange }) => {
    ...
    return (
        <div className={s.paginationWrapper}>
            <button disabled={currentPage === 1} onClick={()=> onPageChange(currentPage-1)} className={s.prev + " " + s.butt2}><Arrow2 />Previous</button>
            ...
            <button disabled={currentPage === totalPages} onClick={()=> onPageChange(currentPage+1)} className={s.next + " " + s.butt2}>Next<Arrow2 /></button>
        </div>
    );
};
